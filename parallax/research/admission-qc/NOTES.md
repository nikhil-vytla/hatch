...
1. **slop-code-bench contributing-problems checklists**
   ...
   exactly what the user asked the skill not to become. Used them for the
   *questions*, not the format.
2. **METR Task Development Guide** (taskdev.metr.org — Desiderata and
...
3. **HUD "Designing tasks"** (docs.hud.ai/v6/reference/advice). The best
   ...
   (root-cause, grader, eval-context, author artifacts) — mechanical lints
   catch verbatim bytes, judgment catches paraphrase and hint; prompt-grader
...
4. **Prime Intellect verifiers v1 GUIDE + Environments Hub docs**
   ...
   `valid`/`invalid`/`timeout`/`error`. The audit's Q2 survey additionally
...
6. **Anthropic, "Demystifying evals for AI agents"**
   ...
   as the non-negotiable practice — metrics say that something changed,
   transcripts say why.
7. **METR Task Standard repo** (github.com/METR/task-standard). Automated
   ...
   score of a known-good and a known-incorrect solution — the same
...
already summarized in the audit table — cited via the audit rather than
re-derived.
...
- **Retry policy: retries are for run failures, never for verdicts.** A
  ...
  `admitted_flaky` recorded — Prime Intellect's flaky/broken separation
