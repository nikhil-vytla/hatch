...
- Host side: `hard-repo-tasks/pyproject.toml` (archive-branch checkout)
  does NOT carry hud; the
...
- This folder was authored as hard-repo-tasks/benchmark-selection-coding/ and
  merged by PR #13 into the archive branch cursor/hard-repo-tasks-5fc8
  (merge commit 43dcbc8), because that branch was the checkout at the time.
  hard-repo-tasks/ is the superseded experiment's namespace and does not
  exist on origin/main.
- Relocated verbatim to parallax/research/benchmark-selection-coding/ per
  the convention in parallax/research/README.md. Only changes: references to
  archive-only sibling paths (architecture/episode-spine, the upstream
  overlay characterization, the root pyproject) now point at or name the
  archive branch explicitly, since they have no on-main referent.
