...
start from internal state: not intents, not event schedules, not
construction transcripts. The agent never sees those, and reading them
...
test, an experiment, or a scripted exercise, which changes agent behavior.
...
proposed fix or the rejection pattern remove sources *selectively*, say
all multi-argument sources or all of one repo, quietly reshaping the
...
- a `Verdict:` line reading `admit`, `admit-with-notes`, or `reject`;
