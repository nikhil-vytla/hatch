...
> model provider over a declared GSM8K sample. The design is worth running when
> the interval it produces would be narrow enough to say anything, which on
> current arithmetic means far more source clusters than any run has used.
