...
2. `CounterfactualGenerator.generate_counterfactuals` calls a provider for
   ...
   variants in `counterfactual_arguments`. BIRD-SQL has a separate
   database-backed deterministic counterfactual generator.
...
7. The SWE-bench Verified wrapper removes symptom arguments before generic
   scheduling, then uses `post_fill_hook` to repair phase ownership and insert
   symptoms before other arguments in their function phase.
...
`fixtures/receipt.json` is generated by running the exact pinned source. The
probe inside it is a symbolic scheduler input, not a provider-generated
benchmark record. It is used only to expose deterministic control flow and
rendering.
