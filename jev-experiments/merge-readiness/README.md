...
The automated audit requests triggered Vercel's ten-minute system challenge for the auditing client. Firewall status showed no custom configuration or pending changes. Owner-authenticated CLI access was used to inspect the immutable production deployment without changing firewall settings. This distinguishes the hosting challenge from application authentication. The scan records its access mode and exact deployment URL. After the challenge expired, normal public HTTPS returned homepage 200 and a live caller-key evaluation 200 on the final deployment, without an owner bypass. See `public-live-check.json`.
