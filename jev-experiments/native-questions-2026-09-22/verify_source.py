#!/usr/bin/env python3
"""Check the native-question slice from one Git archive without local overlays."""
...
CHECKS = [
    ...
    ("adapter frozen install", "adapters/typescript", ["bun", "install", "--frozen-lockfile"]),
    ("adapter TypeScript", "adapters/typescript", ["bunx", "--no-install", "tsc", "--noEmit"]),
    ("native and adapter tests", "adapters/typescript", ["bun", "test", "index.test.ts", "../../packages/decision-runtime/src/native.test.ts"]),
    ("Python frozen install", ".", ["uv", "sync", "--extra", "dev", "--frozen", "--python", "python3"]),
    ("Python contracts", ".", ["uv", "run", "--frozen", "pytest", "-q", "tests"]),
