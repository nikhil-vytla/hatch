...
const report = { checked_at: new Date().toISOString(), complete: records.size === 7440, cases: pairs.length, source_clusters: new Set(pairs.map(p => `${p.source}/${p.original_id}`)).size, completed_evaluations: records.size, planned_evaluations: 7440, verified_questions: verifiedQuestions, planned_questions: 14880, verified_wire_batches: verifiedBatches, raw_normalized_question_agreements: rawAgreement, immutable_successes: true, source_sha256: manifest.cases_sha256, protocol_sha256: manifest.protocol_sha256 };
