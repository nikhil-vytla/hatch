...
export const PROTOCOL = {
  version: "judge-reliability-v1", model: "typesafe-ai/jev", repeats: 3, schedule_seeds: [42, 43, 44],
  pairs: 620, source_questions: 268, response_models: { [MODELS[0]]: 350, [MODELS[1]]: 270 },
  logical_requests: 7440, question_decisions: 14880, questions,
  shared: "Exact historical three-question request, both candidate orientations. Questions share state and are independently evaluated by the documented provider contract. No unrelated states batched together.",
  isolated: "Each canonical candidate alone as state A, with the unchanged a_correct question. One candidate per request; no opponent, source label, source model, correct label, or previous output in state.",
  repeats_control: "Three independently recorded passes with unchanged request hashes. Seeds shuffle scheduling only, not model sampling. Conditions interleaved within each pass; passes run sequentially.",
