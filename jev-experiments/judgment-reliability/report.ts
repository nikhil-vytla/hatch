...
${done ? "Completed" : "Recording in progress"}: ${r.availability.completed.toLocaleString()}/${r.availability.planned.toLocaleString()} evaluation records, covering ${r.availability.completed_decisions.toLocaleString()}/${r.availability.planned_decisions.toLocaleString()} independent question decisions. The frozen study includes all 620 released response pairs, both orders, three unchanged pairwise passes, pair-context whole-answer scoring, and isolated-candidate whole-answer scoring. ${done ? `Pass 1 official two-order accuracy is ${pct(p.official_accuracy)}, compared with ${pct(p.ordered_accuracy)} single-order accuracy and ${pct(p.position_flip_rate)} answer-identity disagreements between the two orders.` : "All scores below are provisional until coverage is complete. Missing evaluations are retained in planned denominators and availability is shown separately."}

## Design and source

[JudgeBench](https://github.com/ScalerLab/JudgeBench/tree/${r.manifest.source_commit}) supplies 350 GPT-4o pairs and 270 Claude 3.5 Sonnet pairs. These are 620 candidate pairs from ${r.analysis.source_clusters} source-question groups, with 308 MMLU-Pro, 239 LiveBench and 73 LiveCodeBench pairs. The source commit is pinned to \`${r.manifest.source_commit}\`; manifest.json stores source-file SHA-256 values, complete input hash, protocol hash and maximum input size. cases.jsonl contains the full original question and both answers plus provenance and content triage. No candidate generation or model training occurred.
...
Analysis amendment ${r.analysis.version} corrects the frozen manifest's erroneous 268-question count. Missing original IDs had grouped unrelated questions together. The source cases, request protocol, recorded decisions and accuracy counts remain unchanged; question grouping and uncertainty intervals are corrected. The original manifest stays available as historical evidence alongside the amendment.

Intervals use 10,000 reproducible bootstrap draws over ${r.analysis.source_clusters} source-question groups, preserving candidate pairs, both response models, orientations and repeats within each group. ${r.analysis.cluster_rule} They estimate variation across broader questions, not uncertainty in the fixed benchmark count. Comparisons use the same original cases. Missing matched sets are excluded from intervals and their counts are explicit.
...
All ${r.analysis.evidence_coverage.normalized_questions.toLocaleString()} question outputs are retained in normalized evaluation records. Of these, ${r.analysis.evidence_coverage.raw_matched_questions.toLocaleString()} also match saved native-batch answer maps exactly; the earliest ${r.analysis.evidence_coverage.normalized_only_batches} accepted batches, containing ${r.analysis.evidence_coverage.normalized_only_questions} questions, predate batch-map logging and retain normalized outputs only.
