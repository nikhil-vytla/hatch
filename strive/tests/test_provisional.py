"""Provisional activations: scoped, monitored, reversible, expiring."""
...
def test_provisional_activation_confirms_when_window_sustains_baseline(
    tmp_path: Path,
) -> None:
    store, generation_id = _prepare(tmp_path, EQUIVALENT_GOOD_STRATEGY)
    activation, decision = promote_generation(
        store, SUM_INTEGERS_TASK, generation_id, provisional=True, expires_after_cycles=2
    )
    assert decision is None  # provisional path takes no paired evidence
    assert activation.mode == ACTIVATION_PROVISIONAL
    assert activation.baseline_score == 1.0
...
def test_provisional_activation_expires_and_reverts_on_regression(
    tmp_path: Path,
) -> None:
