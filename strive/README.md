...
| Models | provider-neutral `ModelAdapter` + deterministic `FakeModelAdapter`; all I/O journaled and budget-metered (no evolution component uses a model yet — next phase) |
...
clean diagnostic, never a traceback. A demo ledger produced by the real CLI is
committed at `artifacts/demo/` (see `transcript.txt` there).
...
│   ├── model.py       provider-neutral model interface + fake adapter
│   ├── diagnose.py    visible-evidence diagnosis (holdout-isolated)
│   ├── propose.py     bounded proposal (holdout-isolated)
│   ├── evaluate.py    split-aware scoring + feedback
│   ├── loop.py        the trusted kernel orchestrator
│   └── cli.py         human + machine-readable CLI
├── tests/             77 tests incl. failure injection — offline, deterministic
├── docs/              charter, architecture, roadmap, handoff, research notes
├── artifacts/demo/    committed demo ledger + transcript
