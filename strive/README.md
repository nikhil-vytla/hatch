...
| Evidence & selection (stage 3C.2A) | **versioned validation evidence and policy-neutral selection for every candidate**: frozen envelopes (`strive.evidence`) — `DatasetRevision@1` (per-split CAS manifests, parent/reason/counts/fingerprint, persisted in `ledger/<task>.datasets.jsonl`; growing a split creates a new revision and a re-evaluation requirement, never a task-drift acknowledgement), `EvaluationManifest@1` (resolved harness, task/dataset fingerprints, environment/scorer, tool versions, runtime, seeds, validator refs, budgets, objective), role-bound `ValidationBundle@1`/`ValidatorResult@1` (flat metrics; per-case payloads as CAS artifacts), typed `DecisionEvidence@1` + `SelectionDecision@1` (closed dispositions promote/reject/frontier_add/provisional_activate — every disposition requires evidence), trusted `ObjectiveSpec@1`, and the `function-task@1` spec adapter. Trusted validator registry resolved by name AND version (unknown either fails closed). The activation gate requires complete CURRENT evidence for the exact revision and active baseline: every changed surface's role (task+constraint always, prompt for prompt deltas — no borrowing or relabeling), current-dataset manifests (stale ⇒ re-baseline), all hard constraints passed (inconclusive blocks), artifacts decodable; activation cites the exact `SelectionDecision`; overrides stay distinct. Migration `0005-evidence-backfill` links pre-envelope history to synthetic-but-lossless envelopes (original refs become the artifacts; nothing rewritten; idempotent). `strive evidence` inspects manifests/bundles/decisions/roles/blocking reasons; replay recomputes bundle metrics and diffs them |
