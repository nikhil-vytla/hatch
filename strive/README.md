...
| Prompt surface (stage 3C.1) | **the second evolvable surface is operational**: `prompt/proposal-template` (descriptor `prompt@2`, materializer kernel-text@1, versioned template validator — bounded, known placeholders, output contract) is resolved from the active revision's manifest for every proposal (`resolve_active_prompt`; the built-in default is the explicit CAS-stored fallback), journaled per model request (prompt ref + active revision + exact consumed prompt bytes), and restored with strategy code by restart and whole-revision rollback. Proposals may carry a bounded `prompt_update` (kernel-screened: template validity + no hidden-split content); the loop builds **one immutable prompt+code candidate revision before evaluation** and the exact evaluated revision is retained/selected/activated/rolled back. `strive experiment` runs the matched-arm causal experiment (A incumbent prompt → gate rejects; B candidate prompt → same adapter/budgets, gate accepts; C prompt-only → rejected; D code-only → accepted; E prompt+code → accepted + activated + restart + rollback) with per-arm scores/regressions/executions/model-calls/tokens/latency; consumption proven from journaled prompt bytes. **The offline fixture proves causal pipeline wiring, not model capability**; `--real-model` (env-configured + `--unsafe-model-code`) records honest real-adapter outcomes, generation-native only |
