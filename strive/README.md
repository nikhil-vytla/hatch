...
| Revision lifecycle (stage 3B.3) | **the canonical owner of native composite revisions** (`strive.lifecycle`, `<task>.revisions.jsonl`), separate from the generation ledger and the generation→revision mirror, written in crash-framed, hash-chained, expected-head batches (shared `strive.framing`). It records `RevisionRetained` (pinning the EXACT evaluated `HarnessRevision` by CAS ref), `RevisionActivation` (active state = the latest valid activation), and a durable `LifecycleBreaker`. The candidate overlay created and validated before evaluation is retained unchanged for **both rejected and accepted** candidates — same `RevisionRef`, manifest, deltas, provenance, descriptors, artifacts — with evaluation/decision evidence linked by versioned CAS ref; on acceptance the SAME revision is activated (evaluated id == retained id == activated id), never a replacement built afterwards. Active state materializes from the **complete `ScopeManifest`** (every surface, e.g. code + prompt), never one source field; the strategy-only `Generation` is an explicitly-derived compatibility projection that lists but never flattens other surfaces. Retention/activation validate the whole revision, scope, descriptors, manifest closure, provenance, parent head, and artifact hashes; whole-revision rollback appends an activation of the prior known-good revision (nothing deleted); invalid composite activation opens the breaker and refuses with no lossy fallback; partial writes recover idempotently. `strive lifecycle [status\|rollback]` |
