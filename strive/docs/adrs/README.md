...
| [0007](0007-sandbox-boundary.md) | The pluggable sandbox boundary and the model-capability lane | **implemented in 3C.2B**: trusted versioned `SandboxBackend` protocol + fail-closed registry (no silent downgrade); `deno-pyodide@1` secure local backend (Deno+Pyodide WASM, default-deny, fresh-per-case, wall-clock hard-kill); NOOA-derived Landlock/seccomp spike; per-case protected evaluation; sandbox provenance in `EvaluationManifest@3`; the `strive.capability` model lane (repeated seeded real-model trials vs fixture controls) |
