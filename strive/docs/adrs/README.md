...
| [0003](0003-tasks-and-environments.md) | Task spec versions, dataset revisions, environments | dataset revisions + evaluation manifests + the `function-task@1` spec adapter implemented in 3C.2A (`strive.datasets`/`strive.evidence`; spec fingerprint excludes cases, growth forces re-baselining); Environment session protocols and Trajectory remain provisional until stage 4 |
| [0004](0004-evidence-and-selection.md) | Validation bundles and selection decisions | **implemented in 3C.2A** (`strive.evidence`/`strive.validators`/`strive.selection`): frozen role-typed envelopes, policy-neutral decisions with the closed disposition vocabulary (frontier_add structural), the activation-evidence gate, and the 0005 lossless backfill; `compare`/replay derive from bundles |
| [0005](0005-evolution-algorithms.md) | Evolution algorithms and objective specs | `ObjectiveSpec@1` implemented in 3C.2A (trusted, pinned on every manifest and decision, not evolvable); the `EvolutionAlgorithm` protocol, KernelServices, and AlgorithmRun/Step remain provisional — NEXT SLICE (3C.2B): budget-matched hill-climb@1 vs pareto-population@1 |
