...
> **vNext reset.** [ADR-0008](0008-vnext-substrate.md) resets the
> architecture around a policy-neutral, revision-native mechanism substrate.
> It **supersedes the empirical-promotion premise** of ADR-0004 and the
> "selection decides every activation" framing of ADR-0005, and retires the
> loop-era `AcceptancePolicy` gate. ADR-0001/0002/0003/0006/0007 are
> refactored (their FLOOR mechanisms survive; the mirror / dual-write /
> generation-authority / migration machinery is deleted). Rows below are
> historical status as of Stage 3C.

| ADR | Title | Status |
|---|---|---|
| [0008](0008-vnext-substrate.md) | vNext: policy-neutral revision-native substrate | **accepted, implemented (Phase A, corrected)** — a run-scoped, semantically-verified event/CAS substrate (`VerifiedSubstrateView`) + a result-driven resumable kernel (`next_command`/`reduce`, one intent/effect/terminal per command, exact resume) + the floor (bound identity, budgets, sandbox capabilities, CAS closure, fork base/candidate refs) + the `strive` CLI; `manual-change@1` proves propose/fork/apply/revert exactly. Supersedes the promotion premise of 0004/0005. Next: `continual-refine@1` |
