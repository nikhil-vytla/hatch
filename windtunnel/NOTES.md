# NOTES — Windtunnel (formerly Orrery)
...
## 2026-07-04 — Rename: orrery → windtunnel

User asked for a new name. Shortlisted against PyPI availability (checked
live): sandtable/kriegspiel/vivarium/diorama are all actively maintained
collisions; free candidates were backlot, windtunnel, seahaven, soundstage,
formicarium. Offered backlot (generation-first brand) vs windtunnel
(stress-testing brand); **user picked windtunnel** — controlled,
instrumented, repeatable turbulence before the design flies.

Mechanics of the rename (earlier entries above still say "orrery" because
that's what happened; history left intact):
- git mv folder + src package; global Orrery/orrery/ORRERY replace outside
  NOTES.md; entry-point groups now "windtunnel.*"; env var is
  WINDTUNNEL_LOG_LEVEL; genesis hash string changed (fingerprints are
  self-consistent per trace, so no compat issue — pre-rename example traces
  were regenerated anyway).
- Opportunistic fix while touching it: TraceMeta.orrery_version →
  **engine_version** (rename-proof field name; the old name would have made
  every stored trace unreadable after any future rebrand).
- Regenerated all example artifacts (spec/trace/sft/adapted) under the new
  name; run + replay + export + adapt re-verified from the CLI.
- Gate: 35/35 tests, ruff clean, pyright clean under the new package name.
