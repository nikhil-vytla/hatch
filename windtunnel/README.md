...
An **windtunnel** is a mechanical model of a planetary system: a small, precise,
...
A complete vertical slice in `src/windtunnel/` (~2.2k lines of library code,
...
- reference world: a support desk with a hidden fraud flag, an NPC customer,
  ...
  contract
- CLI: `windtunnel run | replay | verify | generate`

```
$ uv run windtunnel run --generate support_desk --seeds 5
...
$ uv run windtunnel run --generate support_desk --seeds 3 --brief '{"leak_secret": true}'
  [FAIL] no_fraud_leak: secret mentioned  evidence=['ev-000024']
0/3 runs passed their contract

$ uv run windtunnel replay examples/spec-seed1.json examples/support_desk-1.jsonl
...
uv run windtunnel run worlds/support_desk.toml --seed 7
uv run windtunnel run --generate support_desk --seeds 20 --out runs/   # population QA
```
...
- **QA at release speed**: `windtunnel run --generate X --seeds N` returns a pass
...
- **Any model, any provider** ([models.py](src/windtunnel/models.py), ADR-0007):
...
- **Benchmark ingestion** ([adapters.py](src/windtunnel/adapters.py), ADR-0008):
  ...
  conversion is **oracle-self-validated**: `windtunnel adapt tasks.jsonl --run`
...
| [src/windtunnel/](src/windtunnel/) | the engine (see [docs/architecture.md](docs/architecture.md) module map) |
