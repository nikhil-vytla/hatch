...
`architecture/episode-spine/swebench-environment/` pattern, with one change:
...
- Both spike tests collect under
  ...
  `swebench-environment/` (which pins `hud==0.6.12`). The repo-root
  `hard-repo-tasks` environment does not include hud — use the
