...
| Observability | 🟡 structured JSON logs + `DecisionObserver` hook | OTel spans over the engine loop; trace→OTLP converter | `orrery report` HTML; live run dashboards fed by the observer seam |

## RL / training data

**Position: Orrery is the environment side of RL, never the trainer.** We
...
- `orrery export spec trace --out data.jsonl`, with `require_pass`
...
2. *Token fidelity (M4+).* For RL you must train on exactly the tokens the
   ...
   not fake this with re-tokenization; until then Orrery advertises SFT-grade
   export only.
3. *Rollout service (with T2).* An environment server speaking
   verifiers-style rollout APIs so trainers drive Orrery worlds directly:
...
- `fold(events) == final_state` isn't asserted on read (debt #3-adjacent) —
  a lying `final_state` would go unnoticed by `orrery verify`. Cheap fix, do
  at M3.
...
**Today:** structured JSON logs to stderr (namespaced, `ORRERY_LOG_LEVEL`),
...
2. *Trace→OTLP converter.* Because the trace is complete, any APM can render
   ...
   actors become resources. `orrery otel trace.jsonl` batch-exports.
3. *`orrery report`.* Static HTML per run/population: timeline lanes per
