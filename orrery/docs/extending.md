...
2. **In your package:** expose an entry point in group `orrery.domains`
   (or `orrery.policies` / `orrery.verifiers`); discovered automatically.
...
```python
# my_agent.py
from orrery import Decision, Intent
...
Your agent now runs inside any Orrery world — chaos, hidden state, contracts,
...
the task's known-correct actions). Then `orrery adapt your_format tasks.jsonl
